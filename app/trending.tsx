...
import { getTrendingDramasWithPagination } from '@/services/api';
...
export default function TrendingScreen() {
  ...
  const trendingQuery = useInfiniteQuery<DramaResponse, Error>({
    queryKey: ['trending-dramas-full'],
    queryFn: ({ pageParam = 1 }) => getTrendingDramasWithPagination(pageParam as number),
  ...
  const { data, isLoading, isFetching, isFetchingNextPage, hasNextPage, refetch, fetchNextPage } = trendingQuery;
    ...
    <>
      ...
      <View style={[styles.container, { paddingTop: insets.top }]}>
        {isLoading ? (
          ...
          <FlatList
            data={allDramas}
            keyExtractor={(item, index) => item?.id ? `trending-${item.id}` : `trending-fallback-${index}`}
