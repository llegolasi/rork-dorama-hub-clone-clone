...
interface FounderCollectionModalProps {
  visible: boolean;
  onClose: () => void;
  dramaId: number;
  dramaTitle: string;
...
export default function FounderCollectionModal({
  visible,
  onClose,
  dramaId,
  dramaTitle,
  ...
  const { data: collections, isLoading } = useQuery({
    ...
    queryFn: async () => {
      console.log('[FounderCollectionModal] Fetching all collections');
      ...
      if (error) {
        console.error('[FounderCollectionModal] Error fetching collections:', error);
        throw error;
      }
  ...
  const { data: dramaCollections } = useQuery({
    ...
    queryFn: async () => {
      console.log('[FounderCollectionModal] Fetching drama collections');
      const { data, error } = await supabase
      ...
      if (error) {
        console.error('[FounderCollectionModal] Error fetching drama collections:', error);
        throw error;
      }
  ...
  const createCollectionMutation = useMutation({
    mutationFn: async (data: { title: string; description: string; isVisible: boolean }) => {
      console.log('[FounderCollectionModal] Creating new collection:', data);
      ...
      if (error) {
        console.error('[FounderCollectionModal] Error creating collection:', error);
        throw error;
      }
    ...
    onError: (error) => {
      console.error('[FounderCollectionModal] Create error:', error);
  ...
  const toggleDramaInCollectionMutation = useMutation({
    mutationFn: async ({ collectionId, isInCollection }: { collectionId: string; isInCollection: boolean }) => {
      if (isInCollection) {
        console.log('[FounderCollectionModal] Removing drama from collection');
        ...
        console.log('[FounderCollectionModal] Adding drama to collection');
    ...
    onError: (error) => {
      console.error('[FounderCollectionModal] Toggle error:', error);
